...
<div id="splash" style="position:fixed;inset:0;z-index:20;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#08080c;transition:opacity 0.8s ease,transform 0.8s ease">
<div class="hex-bg"><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span><span>&#x2B21;</span></div>
<div class="starfield"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div>
<div class="scanlines"></div>
<div class="circuit-lines"><svg viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 200 H100 V100 H200 M200 100 V300 H300 V200 H400 M0 300 H150 V350 H250 M250 50 H350 V150 H400" stroke="#f59e0b" stroke-width="0.5" stroke-dasharray="200" class="circuit-trace"/></svg></div>
  ...
  <div class="boot-msg" id="bootMsg" style="margin-top:0.5rem"><span id="bootLine1" class="cursor-blink"></span><span id="bootLine2" style="opacity:0;transition:opacity 0.3s"></span></div>
</div>
<div id="loginContainer" style="display:none;width:100%;align-items:center;justify-content:center">
<div class="bg-glow"></div>
<div class="card" style="animation:loginFadeIn 0.6s ease-out">${LOGO_HTML}<h1>Welcome back</h1><p class="subtitle">Sign in to your Navigwiz account</p><div id="error" class="error"></div><form id="loginForm" novalidate><div class="form-group"><label for="email">Email</label><input id="email" type="email" placeholder="you@example.com" required autocomplete="email" autocapitalize="off" spellcheck="false"></div><div class="form-group"><label for="password">Password</label><div class="pw-wrap"><input id="password" type="password" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" required autocomplete="current-password" minlength="6"><button type="button" class="pw-toggle" id="pwToggle" onclick="togglePw()" tabindex="-1" aria-label="Toggle password visibility"><svg id="eyeIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></div></div><button type="submit" class="btn" id="submitBtn">Sign In</button></form><p class="footer-text">Don\'t have an account? <a href="/signup${rParam}">Create one</a></p></div>
</div>
<script>
var pwVisible=false;function togglePw(){var p=document.getElementById('password'),e=document.getElementById('eyeIcon');pwVisible=!pwVisible;p.type=pwVisible?'text':'password';e.innerHTML=pwVisible?'<path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/>':'<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>'}
(function(){var b=document.getElementById('splash'),l=document.getElementById('loginContainer'),l1=document.getElementById('bootLine1'),l2=document.getElementById('bootLine2'),lb=document.getElementById('loadingBar'),msgs1=["INITIALIZING SECURE CONNECTION...","LOADING AUTHENTICATION MODULES...","ESTABLISHING ENCRYPTION...","VERIFYING INTEGRITY...","CONNECTING TO NAVIGWIZ GATEWAY..."],msgs2=["","Handshake complete","RSA-4096 verified","SHA-256 checksum ok","TLS 1.3 established","Secure tunnel ready"],i=0;function showMsg(){if(i<msgs1.length){l1.textContent=msgs1[i];l1.className='cursor-blink';l2.textContent=msgs2[i]||'';l2.style.opacity=msgs2[i]?'0.5':'0';i++;setTimeout(showMsg,700)}else{l1.textContent='';l1.className='';l2.textContent='';setTimeout(function(){b.style.opacity='0';b.style.transform='scale(1.05)';setTimeout(function(){b.style.display='none';l.style.display='flex';l.style.animation='loginFadeIn 0.6s ease-out'},800)},400)}}showMsg();var e=document.getElementById('loginForm'),t=document.getElementById('email'),n=document.getElementById('password'),o=document.getElementById('error'),i2=document.getElementById('submitBtn'),r=new URLSearchParams(location.search).get('redirect')||'/';e.addEventListener('submit',async function(a){a.preventDefault();o.classList.remove('show');i2.disabled=true;i2.innerHTML='<span class="spinner"></span>Signing in...';try{var d=await fetch('/api/auth/login-redirect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:t.value.trim(),password:n.value,redirect:r})}),s=await d.json();if(s.redirectUrl){window.location.href=s.redirectUrl}else{o.textContent=s.error||'Invalid email or password';o.classList.add('show');i2.disabled=false;i2.textContent='Sign In'}}catch(c){o.textContent='Connection error. Please try again.';o.classList.add('show');i2.disabled=false;i2.textContent='Sign In'}})})();
</script>
</body></html>`;
}
